...
class InvertedResidual(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio, se_ratio=None):
        ...
        # se
        if has_se > 0:
            self.se = SqueezeExcite(hidden_dim, se_ratio=se_ratio, act_layer=nn.PReLU, act_args={"num_parameters": int(hidden_dim*se_ratio)})
        else:
            self.se = None
...
class MobileFaceNet(nn.Module):
    def __init__(self, width_mult=1.0, inverted_residual_setting=None, round_nearest=8,
                 input_size=224, input_channel=3, last_channel=512, se_ratio=None, output_name="GDC", return_features=False):
        """
        ...
        for t, c, n, s in inverted_residual_setting:
            ...
            for i in range(n):
                ...
                features.append(block(input_channel, output_channel, stride, expand_ratio=t, se_ratio=se_ratio))
